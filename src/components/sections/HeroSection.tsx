...
function HeroFixed({ scrollYProgress, range, onCTAClick }: { scrollYProgress: MotionValue<number>; range: [number, number]; onCTAClick: (p: "ios" | "android") => void }) {
  const { t } = useLanguage();
  const [, end] = range;
  const contentOpacity = useTransform(scrollYProgress, [0, end - 0.04, end], [1, 1, 0]);
  const contentScale = useTransform(scrollYProgress, [0, end - 0.04, end], [1, 1, 0.95]);
    ...
    <div className="absolute inset-0 z-[1] bg-white pointer-events-auto">
      <motion.div style={{ opacity: contentOpacity, scale: contentScale }} className="flex h-full items-center justify-center px-6">
        <div className="flex flex-col items-center text-center">
          ...
          <AnimatedText text={t("hero.title")} className="text-[56px] font-bold leading-[1.1] tracking-tight text-gray-950 lg:text-[72px]" delay={0.3} />
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="mt-8 max-w-md whitespace-pre-line text-lg leading-relaxed text-gray-500">
            {t("hero.subtitle")}
          </motion.p>
...
function HeroMobile({ onCTAClick }: { onCTAClick: (p: "ios" | "android") => void }) {
  ...
  return (
    <section id={SECTION_IDS.hero} className="flex min-h-screen items-center justify-center overflow-hidden px-6 pt-20">
      <div className="flex flex-col items-center text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-8 inline-flex items-center rounded-full border border-gray-200 bg-gray-50 px-4 py-1.5 text-xs font-medium text-gray-600">{t("hero.badge")}</motion.div>
        <AnimatedText text={t("hero.title")} className="text-[40px] font-bold leading-[1.1] tracking-tight text-gray-950" delay={0.3} />
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4, duration: 0.8 }} className="mt-6 max-w-md whitespace-pre-line text-base leading-relaxed text-gray-500">{t("hero.subtitle")}</motion.p>
